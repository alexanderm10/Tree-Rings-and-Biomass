...
for(p in 1:length(plots)){
   cols <- which(names(bm.dated.array) %in% trees.use[trees.use$PlotID==plots[p], "TreeID"])
 
   dated.bm[,cols] <- bm.dated.array[,cols]*ross.density.plot[ross.density.plot$PlotID==paste(plots[p]), "Density.Total..stems.m2."] #mean tree * trees/ha (do for Valles only bc sum of trees != plot density; different sampling method than Neil)
...
for(p in 1:length(plots)){
  ...
  all.bm[,cols] <- bm.all.array[,cols]*ross.density.plot[ross.density.plot$PlotID==paste(plots[p]), "Density.Total..stems.m2."] #mean tree * trees/ha (do for Valles only bc sum of trees != plot density; different sampling method than Neil)
  
}

dim(all.bm)

# generating CI's showing the variability present in the diameter reconstruction
# 
# summary(all.bm)
# vlf.tr <- all.bm[,names(all.bm) %in% trees.use[substr(trees.use$TreeID, 1, 3 )== "VLF", "TreeID"]] 
# vuf.tr <- all.bm[,names(all.bm) %in% trees.use[substr(trees.use$TreeID, 1, 3 )== "VUF", "TreeID"]]  
# 
# vlf.tr.bm <-data.frame(Year=as.numeric(row.names(vlf.tr)), SiteID="VLF", 
#                        Mean=apply(vlf.tr, 1, mean, na.rm=T), 
#                        LB=apply(vlf.tr, 1, quantile, 0.025, na.rm=T), 
#                        UB=apply(vlf.tr, 1, quantile, 0.975, na.rm=T)) 
# 
# 
# vuf.tr.bm <-data.frame(Year=as.numeric(row.names(vuf.tr)), SiteID="VUF", 
#                        Mean=apply(vuf.tr, 1, mean, na.rm=T), 
#                        LB=apply(vuf.tr, 1, quantile, 0.025, na.rm=T), 
#                        UB=apply(vuf.tr, 1, quantile, 0.975, na.rm=T)) 


all.tr.bm <- rbind(vlf.tr.bm, vuf.tr.bm)
summary(all.tr.bm)

ggplot(data=all.tr.bm[all.tr.bm$Year<=2011,]) + facet_grid(SiteID ~ .) +
...
row.names(all.increment)
all.samples <- ifelse(!is.na(all.increment), 1, NA)

dated.samples <- ifelse(!is.na(dated.increment), 1, NA)
summary(dated.samples[,1:10])
#colnames(dated.samples) <- names(dated.increment)

sample.depth.vlf <- data.frame(Year=as.numeric(row.names(all.increment)), SiteID="VLF", 
                               Total=rowSums(all.samples[,substr(colnames(all.samples),1,3)=="VLF"], na.rm=T), 
                               Dated=rowSums(dated.samples[,substr(colnames(dated.samples),1,3)=="VLF"], na.rm=T))
#sample.depth$pSample <- 
summary(sample.depth.vlf)


sample.depth.vuf <- data.frame(Year=as.numeric(row.names(all.increment)), SiteID="VUF", 
                               Total=rowSums(all.samples[,substr(colnames(all.samples),1,3)=="VUF"], na.rm=T), 
                               Dated=rowSums(dated.samples[,substr(colnames(dated.samples),1,3)=="VUF"], na.rm=T))
#sample.depth$pSample <- 
summary(sample.depth.vuf)

sample.depth <- rbind(sample.depth.vlf, sample.depth.vuf)
sample.depth$pDated <- sample.depth$Dated / sample.depth$Total
summary(sample.depth)

ggplot(data=sample.depth[sample.depth$Year <=2011,]) + facet_grid(SiteID~.) +
    geom_line(aes(x=Year, y=pDated), size=2)

ggplot(data=sample.depth[sample.depth$Year <=2011,]) + facet_grid(SiteID~.) +
  geom_line(aes(x=Year, y=Total/max(Total)), size=2, color="black") +
  geom_line(aes(x=Year, y=Dated), size=1.5, color="red")
  
sample.depth$Total.adj <- sample.depth$Total/max(sample.depth$Total)*max(valles.inc.stack$inc.UB, na.rm=T)
sample.depth$Dated.adj <- sample.depth$Dated/max(sample.depth$Total)*max(valles.inc.stack$inc.UB, na.rm=T)

ggplot(data=sample.depth[sample.depth$Year <=2011,]) + facet_grid(SiteID~.) +
  geom_line(aes(x=Year, y=Total.adj), size=2, color="black") +
  geom_line(aes(x=Year, y=Dated.adj), size=1.5, color="red")
...
ggplot(data=valles.inc.stack[valles.inc.stack$Year<2012,])  + facet_grid(SiteID ~.) +
  ...
  geom_ribbon(aes(x=Year, ymin=inc.LB, ymax=inc.UB, fill=group), alpha=0.5) +
  geom_line(aes(x=Year, y=Mean.inc, color=group))+
  geom_line(data=sample.depth[sample.depth$Year <=2011,], aes(x=Year, y=Total.adj), size=1, color="red") +
  geom_line(data=sample.depth[sample.depth$Year <=2011,], aes(x=Year, y=Dated.adj), size=1, color="blue") +
  geom_line(data=sample.depth[sample.depth$Year <=2011,], aes(x=Year, y=(1-pDated)*max(valles.inc.stack$inc.UB, na.rm=T)), size=1, color="black") +
  labs(title= "Comparison BM inc", x="Year", y="Bm inc. (kg/m2)")
