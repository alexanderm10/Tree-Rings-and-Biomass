...
# Querying full range
AllomAve(spp.list,2,outdir=file.path(outdir, "0.1-500"),parm="../data/Table3_GTR-NE-319.v2.csv",ngibbs=1000, dmin=0.1, dmax=500)

# Querying double-truncated range
AllomAve(spp.list,2,outdir=file.path(outdir, "10-50"),parm="../data/Table3_GTR-NE-319.v2.csv",ngibbs=1000, dmin=10, dmax=50)

# Querying left-truncated range
AllomAve(spp.list,2,outdir=file.path(outdir, "10-500"),parm="../data/Table3_GTR-NE-319.v2.csv",ngibbs=1000, dmin=10, dmax=500)

# Querying right-truncated range
AllomAve(spp.list,2,outdir=file.path(outdir, "0.1-50"),parm="../data/Table3_GTR-NE-319.v2.csv",ngibbs=1000, dmin=0.1, dmax=50)
...
for(s in spp.list){
	...
	allom.full[[s]] <- mc[[3]][sample(1:nrow(mc[[3]]), size=100, replace=T),]
}

allom.left <- list()
for(s in spp.list){
	load(paste0(allom.dir, "10-500/Allom.", s, ".2.Rdata"))
	allom.left[[s]] <- mc[[3]][sample(1:nrow(mc[[3]]), size=100, replace=T),]
}

allom.right <- list()
for(s in spp.list){
	load(paste0(allom.dir, "0.1-50/Allom.", s, ".2.Rdata"))
	allom.right[[s]] <- mc[[3]][sample(1:nrow(mc[[3]]), size=100, replace=T),]
}

allom.both <- list()
for(s in spp.list){
	load(paste0(allom.dir, "10-50/Allom.", s, ".2.Rdata"))
	allom.both[[s]] <- mc[[3]][sample(1:nrow(mc[[3]]), size=100, replace=T),]
}
# ----------------------------------------------------
dbh.range <- 1:100
...
# ------------------------------------------------
full.final <- left.final <- right.final <- both.final <- list()
allom.final <- data.frame()
for(s in spp.list){
	full.mean <- rowMeans(temp.full[[s]])
	full.ci <- apply(temp.full[[s]], 1, quantile, c(0.025, 0.975)) 	
	full.final[[s]] <- data.frame(DBH=dbh.range, Mean=full.mean, LB=full.ci[1,], UB=full.ci[2,], Allom="Full Distribution", Species=s)

	left.mean <- rowMeans(temp.left[[s]])
	left.ci <- apply(temp.left[[s]], 1, quantile, c(0.025, 0.975)) 	
	left.final[[s]] <- data.frame(DBH=dbh.range, Mean=left.mean, LB=left.ci[1,], UB=left.ci[2,], Allom="Left-Truncated", Species=s)

	right.mean <- rowMeans(temp.right[[s]])
	right.ci <- apply(temp.right[[s]], 1, quantile, c(0.025, 0.975)) 	
	right.final[[s]] <- data.frame(DBH=dbh.range, Mean=right.mean, LB=right.ci[1,], UB=right.ci[2,], Allom="Right-Truncated", Species=s)

	both.mean <- rowMeans(temp.both[[s]])
	both.ci <- apply(temp.both[[s]], 1, quantile, c(0.025, 0.975)) 	
...
allom.final2[allom.final2$UB>30000,"UB"] <- 30000
allom.final2[allom.final2$LB>30000,"LB"] <- 30000
allom.final2[allom.final2$Mean>30000,"Mean"] <- NA
