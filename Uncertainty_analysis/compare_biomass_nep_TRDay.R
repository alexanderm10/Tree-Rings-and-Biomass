...
nep.morgan.annual <- ts(read.table("../NACP/MMS_L2gapfilled_NEE_annual.txt", header = T), end = 2013, frequency = 1)
...
bioB.niwot <- ts(read.table("../NACP/site_biomass_increment_recon_plotB_kg_per_ha.txt",header = T)/10,end = 2012, frequency = 1)    #/10 to convert kg/ha into g/m2
bioC.niwot <- ts(read.table("../NACP/site_biomass_increment_recon_plotC_kg_per_ha.txt",header = T)/10,end = 2012, frequency = 1)
...
jenkins.general <- ts(read.csv("../NACP/MMF_site_BM_inc2.csv", header = T)[,2], end = 2014, frequency = 1)
 
#  all.morgan.increment <- ts(read.csv("MMF_site_BM_inc2.csv", header = T)[,2], end = 2014, frequency = 1)
# summary(all.morgan.increment)

mmf.spp.increment <-read.csv("MMF_spp_mean_inc.csv", header=T) [,2:13]; rownames(mmf.spp.increment) <- read.csv("MMF_spp_mean_inc.csv", header=T)[,1]
row.names(mmf.spp.increment)
mmf.spp.increment <- mmf.spp.increment[order(row.names(mmf.spp.increment), decreasing=F),]

mmf.spp.increment<- ts(mmf.spp.increment, end=2014, frequency=1)
plot(mmf.spp.increment[,1], type="l", col="red")




load("Biomass_by_Species.Rdata")
biom.site.inc <- biom.site.inc[order(names(biom.site.inc), decreasing=F)]

all.morgan.increment <- biom.site.inc
all.morgan.increment <- ts(all.morgan.increment, end=2014, frequency=1)
...
par(new=T)
ts.plot(mmf.spp.increment, ylim=c(0,800))
...
names(mmf.plotting.inc)<- c("ACSA", "NYSY", "ASTR","FAGR", "FRAX", "LITU", "POGR", "QUAL", "QURU", "SAAL", "TIAM", "ULRU", "ALL")
...
ratio.morgan.spp <- ts(forcomp.morgan.spp[,2:13]/forcomp.morgan.spp[,1], start = 1999, end = 2013)
...
names(ratio.morgan.combo)<- c(" All", "ACSA", "NYSY", "ASTR","FAGR", "FRAX", "LITU", "POGR", "QUAL", "QURU", "SAAL", "TIAM", "ULRU")
...
mmf.shapes <- c(10, 15:20, 15:19,0)
mmf.colors <- c("red", "purple","wheat3", "cadetblue3", "orange4", "darkgreen", "blue", "hotpink1", "black", "red1", "brown", "goldenrod1", "medium purple1")
# dodgerblue
# purple
# wheat3
# cadetblue3
...
ggplot(data=ratio.morgan.stack, aes(Species, ratio)) + 
  ...
  scale_y_continuous(name="aNPP : NEP", limit=c(0,1.5)) + scale_x_discrete(name="Species") + 
  stat_summary(fun.y=mean, geom="point", shape="_", size=15, color="black") 
