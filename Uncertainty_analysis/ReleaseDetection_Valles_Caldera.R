...
release.minor <- lorimer.release(x=dated.rw, r=10, p=15, thresh=0.5)
summary(release.minor[,1:15])
# release.minor[,1:5]
write.csv(release.minor, "Trees_ReleaseEvents_MinorRelease.csv", row.names=T)

# Calculating major release events
release.major <- lorimer.release(x=dated.rw, r=10, p=15, thresh=1)
summary(release.major[,1:20])

write.csv(release.major, "Trees_ReleaseEvents_MajorRelease.csv", row.names=T)
