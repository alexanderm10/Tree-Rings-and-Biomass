...
#   scale_fill_discrete(name="Model", labels = c("nt.PICEA.mean", "nt.piaz.mean", "nt.pine.spp", "nt.vcnp.mean", "nt.pine.dom.mean")))
...
valles.plot<- ggplot()  +
  ...
  geom_ribbon(data=vlf.year, aes(x=year, ymin=nt.PICEA.mean - 1.96*nt.PICEA.sd, ymax=nt.PICEA.mean + 1.96*nt.PICEA.sd), alpha=0.15, fill="red")+
  ...
  geom_line(data=vlf.year,  aes(x=year, y=nt.PICEA.mean), size=1.5, colour="red") +
