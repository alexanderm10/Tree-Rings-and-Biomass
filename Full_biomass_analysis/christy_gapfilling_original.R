...
summary(ring.data.mmf$TreeID)
head(ring.data.mmf)

ring.data.mmf <- ring.data.mmf[order(ring.data.mmf$Year),]
ring.data.mmf <- ring.data.mmf[order(ring.data.mmf$TreeID),]
...
# ---------------------------------------
ring.data.oakop <- ring.data[substr(ring.data$PlotID,1,2)=="OO",]
ring.data.oakop <- droplevels(ring.data.oakop) 
# Note: the log link with the gaussian family to prevent fitting negative values requires that you can't actually have 0 growth so we're going to make it really really small instead
ring.data.oakop$RW <- ifelse(ring.data.oakop$RW==0, 1e-6, ring.data.oakop$RW)
summary(ring.data.oakop)


gamm.oakop <- gamm(log(RW) ~ s(Year, bs="cs", k=3, by=TreeID) + DBH..cm., random=list(Species=~1, Site=~1, PlotID=~1), data= ring.data.oakop, na.action=na.omit)

# Saving the GAMM From above so we can load it without having to refit it
save(gamm.oakop, file="GapFilling_gamm_oakop_06.2015.rData")
...
# Making Predicted ring widths
ring.data.oakop$RW.modeled <- exp(predict(gamm.oakop, ring.data.oakop))
summary(ring.data.oakop)
write.csv(ring.data.oakop, "GapFilling_OAKOP_Measured_Filled.csv", row.names=F)

# Graphing of the modeled rings we will use to fill the data (note this isn't truncating ones that are past where we think pith actually is)
pdf("gamm_gapfill_oakop.pdf", height=7.5, width=10)
ggplot() + facet_wrap(~Species) +
	geom_path(aes(x=Year, y=RW, color=Species), data= ring.data.oakop, size=0.5) +
	geom_point(aes(x=Year, y=RW.modeled), ring.data.oakop[is.na(ring.data.oakop $RW),], size=0.5) +
	scale_y_continuous(limits=c(0,1.25)) +
	theme_bw()
dev.off()
